...
#ifndef EADAPTOR_FEASIBILITY_FIRMWARE_MPU9250_H_
#define EADAPTOR_FEASIBILITY_FIRMWARE_MPU9250_H_

/*#include <stdint.h>
//#include <MSP430.h>

#define MPUCLOCK			12//8

uint16_t MPU_WriteReg(uint8_t WriteAddr, uint8_t WriteData);
uint8_t MPU_ReadReg(uint8_t WriteAddr);
...
uint16_t Set_Gyro_Scale(int scale);
uint16_t Set_Acc_Scale(int scale);
void Calib_Acc();
...
void Read_Acc_OFFSETX();

void Read_All();
//void MPU9250_Spi_Set();
//void Spi_Write(uint8_t *Add,uint8_t length);
void Spi_Select();
void Spi_Deselect();

void MPU9250SelfTest(float * destination);

typedef void (*Void_MPU_Fptr_t)(void);
typedef uint16_t (*Read16_MPU_Fptr_t)(int);
typedef uint16_t (*Int16_MPU_Fptr_t)(void);
typedef uint8_t (*Read8_MPU_Fptr_t)(int);
typedef uint8_t (*Int8_MPU_Fptr_t)(uint8_t);

typedef struct MPU_Instance
{
	Int8_MPU_Fptr_t GetReady;
	Void_MPU_Fptr_t TempData;
	Void_MPU_Fptr_t	AcceleroData;
	Void_MPU_Fptr_t	GyroData;
	Void_MPU_Fptr_t	MagnetoData;
	Read16_MPU_Fptr_t SetGyro;
	Read16_MPU_Fptr_t SetAccelero;
	Void_MPU_Fptr_t	CalibAccelero;
	Void_MPU_Fptr_t	CalibMagneto;
	Int16_MPU_Fptr_t MPUWhoAmI;
	//unsigned int (*MPUWhoAmI)(void);
	uint8_t (*MagnetoWhoAmI)(void);
	Void_MPU_Fptr_t	AllData;
}MPU_Instance;

extern MPU_Instance MPU9205;
...
extern int Accelerometer_Data_MPU1[3],Accelerometer_Data_MPU2[3];;//float Accelerometer_Data_MPU1[3],Accelerometer_Data_MPU2[3];
extern float Temperature_MPU1,Temperature_MPU2;
extern int Gyroscope_Data_MPU1[3],Gyroscope_Data_MPU2[3];//float Gyroscope_Data_MPU1[3],Gyroscope_Data_MPU2[3];
extern int Magnetometer_MPU1[3],Magnetometer_MPU2[3];;//float Magnetometer_MPU1[3],Magnetometer_MPU2[3];
...
#endif /* EADAPTOR_FEASIBILITY_FIRMWARE_MPU9250_H_ */
